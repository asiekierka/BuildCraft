...
public abstract class TileEngine extends TileBuildCraft implements IPowerReceptor, IPowerEmitter, IOverrideDefaultTriggers, IPipeConnection {
    ...
    private int getPowerToExtract() {
        ...
        if (tile instanceof IPowerReceptor) {
            ...
            return (int) extractEnergy(receptor.getMinEnergyReceived(), receptor.getMaxEnergyReceived(), false);
        } else if (tile instanceof IEnergyHandler && ((IEnergyHandler) tile).canConnectEnergy(orientation.getOpposite())) {

            IEnergyHandler handler = (IEnergyHandler) tile;
            int max = handler.receiveEnergy(orientation, (int) (maxEnergyExtracted()*10), true);
            return (int) extractEnergy(0, max / 10, false);
        } else
            return 0;
    ...
    private void sendPower() {
        ...
        if (isPoweredTile(tile, orientation)) {
            ...
            if (tile instanceof IPowerReceptor) {
                ...
                if (extracted > 0) {
                    IEnergyHandler handler = (IEnergyHandler) tile;
                    int max = handler.receiveEnergy(orientation, (int) (maxEnergyExtracted()*10), false);
                    extractEnergy(0, max / 10, true);
                }
            }
